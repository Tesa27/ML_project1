...
def feature_exp_expansion(data, column_names, target_columns):
    ...
    for i in target_columns:
        ...
        if column_names[i] in ['PRI_jet_num0', 'PRI_jet_num1', 'PRI_jet_num2', 'PRI_jet_num3']:
            continue
...
def feature_polynomial_expansion(data, column_names, target_column, degree):
    ...
    if column_names[target_column] in ['PRI_jet_num0', 'PRI_jet_num1', 'PRI_jet_num2', 'PRI_jet_num3', 'Constant']:
        return new_data, new_column_names
    for d in range(2,degree+1):
        new_column = new_data[:,[target_column]] ** d
        new_data = np.concatenate((new_data, new_column), axis=1)
...
def make_population(population_size, column_names, max_iters):
    ...
    for i in range(population_size):
        ...
        for key in column_names:
            if 'PRI_jet_num' in key or key == 'Constant':
                param[key] = 1
            else:
...
def make_offspring(parent1, parent2, mutation_rate=0.05):
    ...
    for i,key in enumerate(parent1.keys()):
        ...
        if mutation_indicator > mutation_rate:
            ...
            elif 'PRI_num_jet' in key or key == 'Constant':
...
def update_statistics(statistics, train, test):
    """Helping function to keep track of metrics"""
    if statistics == {}:
        statistics = {'train_accuracy':np.array([]), 'train_f1':np.array([]), 'train_recall':np.array([]), 'train_precision' :np.array([]),
                      'test_accuracy':np.array([]), 'test_f1':np.array([]), 'test_recall':np.array([]), 'test_precision' :np.array([])
                     }
    statistics['train_accuracy'] = np.append(statistics['train_accuracy'], train['accuracy'])
    statistics['train_f1'] = np.append(statistics['train_f1'],train['f1_score'])
    statistics['train_recall'] = np.append(statistics['train_recall'], train['recall'])
    statistics['train_precision'] = np.append(statistics['train_precision'], train['precision'])
    statistics['test_accuracy'] = np.append(statistics['test_accuracy'], test['accuracy'])
    statistics['test_f1'] = np.append(statistics['test_f1'] , test['f1_score'])
    statistics['test_precision'] = np.append(statistics['test_precision'], test['precision'])
    statistics['test_recall'] = np.append(statistics['test_recall'], test['recall'])
...
def optimize(y, train_data, column_names, GD, population_size=20, size=10, max_iters=40, log=None):
    ...
    max_alg_iters = 1000 if GD else 1000000
    population = make_population(population_size, column_names, max_alg_iters) 
    ...
    # Dictionary that stores information about metrics
    statistics = {}
    ...
    for i in range(max_iters):
        ...
        for param in population:
            ...
            if log != None:
                ...
                f.write('\n-------------------------------------------------------------------\n')
                statistics = update_statistics(statistics, train_evaluation, test_evaluation)
...
def mainModel():
    ...
    yb_train, train_data, ids = load_csv_data(data_path_train, sub_sample=False)
    yb_test, test_data, ids_test = load_csv_data(data_path_test, sub_sample=False)
    ...
    train_data_backup = np.copy(train_data)
    test_data_backup = np.copy(test_data)
    
    #Running the genetic algorithm in order to find hyperparameters for GD
    GD = True
    fitness, w, statistics = optimize(yb_train, train_data, column_names, GD, population_size=8, size=4, max_iters=10, log="GD.txt")
    
    with open("statisticsGD.txt", 'w+') as stat_f:
        stat_f.write(str(statistics))
    ...
    test_data = transform_data(test_data, column_names, best_hyperparameters)

    w, _ = reg_logistic_regression_GD(yb_train, train_data, lambda_, best_w, 1000, gamma)
    ...
    # Making a CSV submission
    create_csv_submission(ids_test, y_predict, 'submissionGD.csv')
    
    # Plotting the distribution of train and test accuracies from all the configurations considered in genetic algorithm
    plt.hist(statistics['train_accuracy'], label = 'train accuracy', color='r', alpha=0.5)
    plt.hist(statistics['test_accuracy'], label = 'test accuracy', color='b', alpha=0.5)
    ...
    # Running the genetic algorithm in order to find hyperparameters for SGD
    train_data = train_data_backup
    test_data = test_data_backup
    GD = False
    fitness, w, statistics = optimize(yb_train, train_data, column_names, GD, population_size=8, size=4, max_iters=10, log="SGD.txt")
    
    with open("statisticsSGD2.txt", 'w+') as stat_f:
        stat_f.write(str(statistics))
    ...
    w, _ = reg_logistic_regression(yb_train, train_data, lambda_, best_w, 1000000, gamma)
    ...
    # Making a CSV submission
    create_csv_submission(ids_test, y_predict, 'submissionSGD2.csv')
...
def main():
    ...
    #We calculate the least squares to obrain the loss and w
    w,loss = least_squares(yb_train, train_data)
    
    #We make our predictions on the test set
    y_predict = predict(test_data,w)
    
    #In this scenario 'b' particles are depicted as zeros
    y_predict[np.where(y_predict == 0)] = -1
    
    #Create the submission csv
    create_csv_submission(ids_test, y_predict, 'submissionLeastSquares.csv')



if __name__ == "__main__":
    # Our main model but does not give the best results 
    #mainModel()
    #The second model that we considered that gives slighlty better result but was not our main focus
    main()
